...
export function PrintableOrder({ order, branch }: PrintableOrderProps) {
  ...
  const PageOne = () => (
    <div className="font-sans text-xs text-black bg-white p-4">
        {/* Header */}
        <header className="flex justify-between items-start pb-4 border-b border-gray-400">
            ...
            <h2 className="font-bold text-lg">Orden de Reparación (Interna)</h2>
        ...
        {/* Details Section */}
        <div className="grid grid-cols-3 gap-2 my-2 text-[9pt]">
            ...
            <div className="border border-gray-300 p-2 rounded">
             ...
             <div className="border border-gray-300 p-2 rounded">
                <h3 className="font-bold border-b border-gray-200 pb-1 mb-1 text-center">Falla y Observaciones</h3>
                <p><strong>Falla Declarada:</strong> {order.declaredFault || "N/A"}</p>
                <p><strong>Observaciones:</strong> {order.observations || "N/A"}</p>
            </div>
        </div>

        {/* Technical Checklist for internal use */}
        <div className="border border-gray-300 p-2 rounded mb-3">
            <h3 className="font-bold border-b border-gray-200 pb-1 mb-2 text-center">Checklist Técnico Detallado</h3>
        ...
        {/* Footer for internal page */}
        <footer className="mt-4 pt-2 border-t text-center text-gray-500 text-[8pt]">
            Uso Interno - NexusServ 360
        </footer>
    </div>
  );
  ...
  const PageTwo = () => (
     <div className="font-sans text-xs text-black bg-white p-4">
        ...
        <header className="flex justify-between items-start pb-4 border-b border-gray-400">
            <div className="flex items-center gap-4">
            {settings?.companyLogoUrl ? (
                <Image src={settings.companyLogoUrl} alt={`${settings.companyName || ''} Logo`} width={100} height={35} className="object-contain" data-ai-hint="company logo" />
            ) : (
                <div className="w-[100px] h-[35px] bg-gray-200 flex items-center justify-center"><span className="text-gray-500 text-xs">Logo</span></div>
            )}
            <div>
                <h1 className="font-bold text-base">{settings?.companyName || "Nombre de la Tienda"}</h1>
                <p className="text-[9pt]">{settings?.companyAddress || "Dirección no especificada"}</p>
